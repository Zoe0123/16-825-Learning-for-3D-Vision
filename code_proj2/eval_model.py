...
def save_plot(thresholds, avg_f1_score, args):
    ...
    plt.savefig(f'eval_{args.type}_10000', bbox_inches='tight')
...
def evaluate_model(args):
    ...
    if args.load_checkpoint:
        checkpoint = torch.load(f'/mnt/data/checkpoint_{args.type}_10000.pth') # changed this!! originally f'checkpoint_{args.type}.pth'
    ...
    for step in range(start_iter, max_iter):
        ...
        # TODO:
        # if (step % args.vis_freq) == 2: # 0
        img_step = step % 50 
        num = step // 50 
        if img_step == 0:
            if args.type == "vox":
                render_vox(predictions[0], output_path=f'images/q_2-1-pred-{num}.gif')
                vox_gt = feed_dict['voxels'].to(args.device)
                render_vox(vox_gt[0], output_path=f'images/q_2-1-gt-{num}.gif')
            if args.type == "point":
                render_pc(predictions, output_path=f'images/q_2-2-10000-pred-{num}.gif')
                gt_points = sample_points_from_meshes(mesh_gt, args.n_points)
                render_pc(gt_points, output_path=f'images/q_2-2-10000--gt-{num}.gif')
            if args.type == "mesh":
